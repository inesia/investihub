...
export default function ManulifeLoginForm() {
  const { login } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await login(email, password, { requireTenant: "manulife" });
    if (result.error) setError(result.error);
    setIsLoading(false);
  };

  const fillDemo = () => {
    setEmail("client@manulife.co.id");
  ...
  return (
    <div className="flex min-h-screen">
      {/* Manulife brand panel */}
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-primary p-12 lg:flex">
        <div
          className="pointer-events-none absolute inset-0 opacity-30"
          style={{
            backgroundImage:
              "radial-gradient(ellipse 80% 60% at 20% 20%, #00A758 0%, transparent 55%), radial-gradient(ellipse 70% 50% at 90% 80%, #007A2B 0%, transparent 50%)",
          }}
        />
        <div className="relative z-10 space-y-3">
          <BrandLogo tenant="manulife" inverted />
        ...
        <div className="relative z-10">
          ...
          <p className="mt-4 max-w-md text-white/65">
            Secure access for PT Asuransi Jiwa Manulife Indonesia — track claim progress,
            review reporting timelines, and collaborate with investigators.
          </p>
        </div>
        <p className="relative z-10 text-sm text-white/45">
          &copy; 2026 Manulife · Operated by PT. Global Investigasi
        </p>
      </div>
      ...
      <div className="flex flex-1 items-center justify-center bg-background p-6">
        ...
        >
          <div className="mb-8 space-y-3">
            <BrandLogo tenant="manulife" className="mb-1" />
          ...
          <h1 className="text-2xl font-bold text-foreground">
            Manulife Client Sign In
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Exclusive portal for PT Asuransi Jiwa Manulife Indonesia clients
          </p>
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <div className="space-y-2">
              ...
              <Input
                id="email"
                type="email"
                placeholder="you@manulife.co.id"
                value={email}
          ...
          <div className="mt-6 rounded-lg border border-dashed border-primary/25 bg-accent/50 p-4">
            <p className="mb-2 text-xs font-medium text-primary">
              Demo Manulife client (password: password123)
            </p>
            <button
              type="button"
              onClick={fillDemo}
            ...
            >
              ...
              <span className="mt-0.5 block text-muted-foreground">
                client@manulife.co.id
              </span>
            </button>
          </div>

          <p className="mt-6 text-center text-sm text-muted-foreground">
            Not an Manulife client?{" "}
            <Link
              href="/login"
